...
- **Claude Code 风格面板界面** — CLI 全面视觉升级
  - 边框面板布局（`╭── title ──╮` / `│ content │` / `╰──╯`）
  - 像素风 mascot 吉祥物（左栏）+ 命令提示（右栏）双栏排版
  - 紫→青渐变色标题栏
  - 首次运行、已配置主屏、setup、guide 四个入口全部面板化

### 修复

- **命令前缀统一为 `adoback`** — 不再显示 `adoback.py`，无论通过哪种方式调用

### 变更

- 移除旧 ASCII art banner，替换为紧凑 mascot + panel 布局
- 新增 `_panel()` 通用面板渲染器（支持单栏/双栏、自动宽度）
- 新增 `_print_mascot_colored()` 主题色 mascot 生成
- 版本号升级到 0.5.6
